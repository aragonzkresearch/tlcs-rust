...
use anyhow::anyhow;
use ark_bls12_381::{ g1, g2, G1Affine as G1Affine_bls, G2Affine as G2Affine_bls,
    G1Projective as G1Projective_bls, G2Projective as G2Projective_bls,
                     g1::Config as G1Config, g2::Config as G2Config
};

use ark_ec::{hashing::{curve_maps::wb::WBMap, map_to_curve_hasher::MapToCurveBasedHasher, HashToCurve,HashToCurveError},
             models::short_weierstrass, pairing::Pairing, pairing::PairingOutput, CurveGroup, AffineRepr,
...
pub fn hash_2<E : Pairing>(
    ...
    let mut compressed_bytes = Vec::new();
    pk_0_vec.serialize_compressed(&mut compressed_bytes).unwrap();
    hasher.update(compressed_bytes);

    let mut compressed_bytes = Vec::new();
    pk_1_vec.serialize_compressed(&mut compressed_bytes).unwrap();
    hasher.update(compressed_bytes);
